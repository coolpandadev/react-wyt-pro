...
const Trade = () => {
    ...
    return (
        <div className='flex flex-col px-4 max-w-screen-xl m-auto pb-20'>
            ...
            <div className="pt-4 flex-col md:flex-row justify-between">
                <p>{tradeInfo?.league?.league_name}</p>
                <p className="text-slate-400">{tradeInfo?.league?.scoring_type} - {tradeInfo?.league?.num_teams} TEAM</p> 
            </div>
            <div className="flex flex-col md:flex-row md:pt-8 justify-center">
            ...
            <div className="comments w-full h-auto pt-8 mb-8">
                ...
                {comments && comments?.length !== 0 ? comments.map((comment) => <div key={comment?.id} className="flex flex-col py-2 items-start gap-x-4 border-b border-slate-200">
                    ...
                    {<p className="normal-case pt-2 text-slate-700 text-sm">{comment?.description}</p>}
                    {/* <p className="text-gray-400">{`${comment.created_at.split("T")[0]} at ${comment.created_at.split("T")[1].slice(0, -8)}`}</p> */}
                </div>) : <h3 className="text-center py-2">No Comments yet</h3>}
            </div>
            <form onSubmit={(e) => handleSubmit(e)} className="flex flex-col">
                {!(isLoggedIn) && <div className='flex'>
                    <input type='text' name='name' ref={commentNameRef} placeholder="Name" className='rounded-md bg-stone-100 resize-none w-full py-2 px-4 text-xs normal-case'/>
                </div>}
                <div className='flex'>
                    <textarea className='rounded-md bg-stone-100 resize-none w-full h-20 mt-4 px-4 py-2 text-xs' ref={commentDescriptionRef} placeholder="Comment here" />
                </div>
                <Button cb={handleSubmit} color="bg-emerald-500" type="submit" classnames="text-white self-end mt-4">Submit</Button>
            </form>
